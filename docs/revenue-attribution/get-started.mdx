...
1. **Track visitors** - seeMoreThanMMR tracks all visitors to your site
2. **Record payments** - When a payment is completed, it's recorded in seeMoreThanMMR
...
Webhooks automatically send payment data to seeMoreThanMMR when a payment is completed. This is the most reliable method.
...
const payment = await stripe.checkout.sessions.create({
  // ... payment config
  metadata: {
    visitorId: window.seeMoreThanMMR?.getVisitorId(),
    sessionId: window.seeMoreThanMMR?.getSessionId(),
  },
});
```
