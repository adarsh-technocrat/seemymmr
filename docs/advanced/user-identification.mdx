...
When a user logs in, identify them with seeMoreThanMMR:

```javascript
seeMoreThanMMR("identify", {
  userId: "user_123",
...
async function handleLogin(email, password) {
  ...
  // Identify user with seeMoreThanMMR
...
User identification is optional. You can use seeMoreThanMMR without identifying users. When users are identified, their data is stored securely and only accessible to you.
