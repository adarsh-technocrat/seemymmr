---
title: "Next.js"
description: "Proxy seeMoreThanMMR with Next.js"
---

Proxy seeMoreThanMMR through your Next.js application.
...
export async function GET(request: NextRequest) {
  ...
  // Forward to seeMoreThanMMR
  const seemorethanmmrUrl = `https://your-seemorethanmmr-domain.com/api/track.js?site=${trackingCode}`;

  try {
    const response = await fetch(seemorethanmmrUrl);
...
export async function GET(request: NextRequest) {
  const searchParams = request.nextUrl.searchParams;
  const seemorethanmmrUrl = `https://your-seemorethanmmr-domain.com/api/track?${searchParams.toString()}`;
...
<Card title="Proxy setup guide" icon="shield" href="/proxy/get-started">
  Learn more about proxying seeMoreThanMMR
</Card>
